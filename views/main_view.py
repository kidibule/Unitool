...
class MainView(ctk.CTkFrame):
    def __init__(self, parent, controller):
        ...
        self.sidebar = ctk.CTkFrame(self, width=200, corner_radius=0, fg_color=DrakeConfig.BG_PANEL)
        self.sidebar.grid(row=0, column=0, sticky="nsew")
        
        # Logo/Titre
        ctk.CTkLabel(self.sidebar, text="Unitool", font=("Orbitron", 28, "bold"), 
                     text_color=DrakeConfig.ACCENT_PRIMARY).pack(pady=(30, 0))
        ctk.CTkLabel(self.sidebar, text="TACTICAL SYSTEMS", font=("Orbitron", 10), 
                     text_color=DrakeConfig.TEXT_SECONDARY).pack(pady=(0, 30))
        ...
        self.intel_panel = ctk.CTkFrame(self, width=280, fg_color=DrakeConfig.BG_PANEL, corner_radius=0)
        self.intel_panel.grid(row=0, column=2, sticky="nsew", padx=0, pady=0)
        
        # Séparateur visuel gauche
        line = ctk.CTkFrame(self.intel_panel, width=2, fg_color=DrakeConfig.BORDER_COLOR)
        line.pack(side="left", fill="y")

        ctk.CTkLabel(self.intel_panel, text="BATTLE INTEL", 
                     font=DrakeConfig.FONT_UI, 
                     text_color=DrakeConfig.ACCENT_PRIMARY).pack(pady=20)
        ...
        ctk.CTkLabel(self.intel_panel, text="VER 3.1.2 - OPS STATUS: NOMINAL", 
                     font=("Consolas", 9), text_color="#444").pack(side="bottom", pady=10)

        self.show_page("ScannerFrame")

    def create_stat_widget(self, title, value):
        """Crée un bloc de données stylisé Drake"""
        f = ctk.CTkFrame(self.intel_panel, fg_color=DrakeConfig.BG_TERMINAL, 
        ...
        ctk.CTkLabel(f, text=title, font=("Segoe UI", 9, "bold"), 
                     text_color=DrakeConfig.TEXT_SECONDARY).pack(pady=(5, 0))
        
        lbl = ctk.CTkLabel(f, text=value, font=("Orbitron", 22, "bold"), 
    ...
    def refresh_intel(self):
        ...
        try:
            ...
            self.stat_threats.configure(text=f"{enemies:03d}", 
                                        text_color=DrakeConfig.ACCENT_ERROR if enemies > 0 else DrakeConfig.ACCENT_PRIMARY)
