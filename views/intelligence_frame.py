...
class IntelligenceFrame(ctk.CTkFrame):
    ...
    def setup_player_tab(self):
        ctk.CTkLabel(self.tab_player, text="TARGET ACQUISITION", font=("Orbitron", 16, "bold"), text_color=DrakeConfig.ACCENT_PRIMARY).pack(pady=5)
        
        self.ent_p = ctk.CTkEntry(self.tab_player, placeholder_text="HANDLE...", width=300, 
                                  fg_color=DrakeConfig.BG_PANEL, border_color=DrakeConfig.ACCENT_PRIMARY)
        self.ent_p.pack(pady=5)
        
        btn_f = ctk.CTkFrame(self.tab_player, fg_color="transparent")
        btn_f.pack(pady=5)
        
        self.btn_scan_p = DrakeButton(btn_f, text="SCANNER", width=100, command=lambda: self.start_work("player"))
        self.btn_scan_p.pack(side="left", padx=5)
        
        self.btn_save_db = DrakeButton(btn_f, text="ENREGISTRER CONTACT", width=150, 
                                       fg_color="#333", state="disabled", command=self.save_to_unitool)
        self.btn_save_db.pack(side="left", padx=5)
        
        # Panel d'affichage des résultats
        self.info_f = ctk.CTkFrame(self.tab_player, fg_color=DrakeConfig.BG_TERMINAL, border_width=1, border_color=DrakeConfig.ACCENT_PRIMARY)
    ...
    def create_row(self, parent, label):
        ...
        ctk.CTkLabel(r, text=label, font=DrakeConfig.FONT_UI, width=150, anchor="w", text_color=DrakeConfig.TEXT_SECONDARY).pack(side="left")
        val = ctk.CTkLabel(r, text="---", font=DrakeConfig.FONT_LOGS, text_color=DrakeConfig.ACCENT_PRIMARY)
        val.pack(side="left")
        return val

    def setup_org_tab(self):
        ctk.CTkLabel(self.tab_org, text="ORGANIZATION MAPPING", font=("Orbitron", 16, "bold"), text_color=DrakeConfig.ACCENT_PRIMARY).pack(pady=5)
                
        self.ent_o = ctk.CTkEntry(self.tab_org, placeholder_text="SID...", width=200, 
                                  fg_color=DrakeConfig.BG_PANEL, border_color=DrakeConfig.ACCENT_PRIMARY)
        ...
        DrakeButton(btns_org, text="EXTRAIRE MEMBRES", command=lambda: self.start_work("org")).pack(side="left", padx=5)
        self.btn_save_org = DrakeButton(btns_org, text="EXPORTER CSV", fg_color="#333", command=self.save_org_to_csv)
        self.btn_save_org.pack(side="left", padx=5)
        
        # TERMINAL 1 : La liste des membres
        self.member_list = DrakeTerminal(self.tab_org, height=180, border_color=DrakeConfig.ACCENT_PRIMARY)
        ...
        self.details_box = DrakeTerminal(self.tab_org, height=100, border_color=DrakeConfig.ACCENT_PRIMARY)
    ...
    def init_driver(self):
        try:
            if not self.bot_driver:
                ...
                self.bot_driver = webdriver.Chrome(service=Service(ChromeDriverManager().install()), options=opts)
            return True
        ...
        try:
            if self.init_driver():
                if mode == "player":
                    ...
                    d = {"Handle": target, "OrgaNom": "NONE", "SID": "N/A", 
                         "Rang": "N/A", "Date": "N/A", "Language": "N/A"}

                    # --- ÉTAPE 1 : PAGE PROFIL ---
                    self.log_box.log(f"scanning profile: {target}...")
                    self.bot_driver.get(f"https://robertsspaceindustries.com/citizens/{target}")
                    time.sleep(4)
                    
                    try:
                        # Utilisation de la méthode body.text (très stable)
                        body_text = self.bot_driver.find_element(By.TAG_NAME, "body").text
                    ...
                    self.bot_driver.get(f"https://robertsspaceindustries.com/citizens/{target}/organizations")
                    ...
                    try:
                        ...
                        org_links = self.bot_driver.find_elements(By.XPATH, "//a[contains(@href, '/orgs/')]")
                        ...
                        for link in org_links:
                            ...
                            if name_text and len(name_text) > 1:
                                ...
                                d["SID"] = link.get_attribute("href").split('/')[-1].upper()
                                found = True
                        ...
                        if found:
                            try:
                                d["Rang"] = self.bot_driver.find_element(By.CSS_COLOR, ".rank-name").text.strip()
                            except: pass
                        else:
                    ...
                    self.btn_save_db.configure(state="normal", fg_color=DrakeConfig.ACCENT_PRIMARY)
                    self.log_box.log(f"intel acquired: {target}")

                elif mode == "org":
                    self.clean_org_info()
                    sid = target.strip().upper()
                    self.bot_driver.get(f"https://robertsspaceindustries.com/orgs/{sid}/members")
                    time.sleep(5)
                    self.bot_driver.execute_script("window.scrollTo(0, 1000);")
                    time.sleep(2)

                    members = self.bot_driver.find_elements(By.CSS_SELECTOR, "li.m-member, .member-item")
                    if members:
                        self.member_list.delete("1.0", "end")
                        self.member_list.insert("end", f"{'HANDLE':<25} | {'RANK':<20}\n{'-'*50}\n")
                        v_count, r_count = 0, 0
                        for m in members:
                            try:
                                try: h = m.find_element(By.CSS_SELECTOR, ".nick").text.strip()
                                except: h = ""
                                if not h:
                                    h, r_count = "[REDACTED]", r_count + 1
                                else:
                                    v_count += 1
                                try: r = m.find_element(By.CSS_SELECTOR, ".rank").text.strip()
                                except: r = "MEMBER"
                                self.member_list.insert("end", f"{h:<25} | {r:<20}\n")
                            except: r_count += 1
                        self.member_list.insert("end", f"{'-'*50}\nTOTAL: {v_count+r_count} | VISIBLE: {v_count} | REDACTED: {r_count}\n")
    ...
    def save_to_unitool(self):
        if self.last_scanned_data:
            ...
            self.btn_save_db.configure(fg_color="#2ecc71", text="ARCHIVÉ ✓", state="disabled")
        ...
        try:
            ...
            if "|" in line_content and "HANDLE" not in line_content:
                ...
                if local_data:
                    ...
                    self.details_box.log(f"NO LOCAL DATA FOR {handle}. DOUBLE-CLICK TO SCAN.")
        except Exception as e:
            pass