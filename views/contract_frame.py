...
class ContractFrame(ctk.CTkFrame):
    def __init__(self, parent, controller):
        ...
        # --- FORMULAIRE D'AJOUT ---
        f_add = ctk.CTkFrame(self, fg_color=DrakeConfig.BG_PANEL, 
        ...
        self.reward_in = ctk.CTkEntry(f_add, placeholder_text="REWARD aUEC", width=120, **entry_kwargs)
       ...
       # Menu Type
        ...
        self.prio_menu = ctk.CTkOptionMenu(f_add, variable=self.prio_var, width=100,
                                           fg_color=DrakeConfig.BG_MAIN,
                                           button_color=DrakeConfig.BORDER_COLOR,
                                           text_color=DrakeConfig.TEXT_MAIN, corner_radius=0,
        ...
        ctk.CTkButton(self, text="EDIT TYPES", command=self.open_type_manager, 
                      fg_color="transparent", border_width=1, border_color=DrakeConfig.BORDER_COLOR, 
                      text_color=DrakeConfig.TEXT_SECONDARY, hover_color=DrakeConfig.BG_PANEL,
                      width=80, height=20, font=("Segoe UI", 9, "bold")).pack(anchor="ne", padx=20)

        # --- SECTION MISSIONS ACTIVES ---
        self.active_scroll = ctk.CTkScrollableFrame(self, label_text="ACTIVE MISSIONS / URGENT TASKS", 
        ...
        # --- STATS & HISTORIQUE ---
        self.stats_label = ctk.CTkLabel(self, text="TOTAL COLLECTED : 0 aUEC", 
                                        font=DrakeConfig.FONT_UI, 
                                        text_color=DrakeConfig.ACCENT_PRIMARY)
        self.stats_label.pack(pady=(10, 0))
        
        self.history_scroll = ctk.CTkScrollableFrame(self, label_text="TRANSACTION LOGS / COMPLETED", 
                                                    label_font=DrakeConfig.FONT_UI, 
                                                    label_fg_color=DrakeConfig.BG_PANEL, 
    ...
    def add_contract(self):
        ...
        if target:
            ...
            if not reward and self.type_var.get() != "TYPE":
                row = self.controller.db.query("SELECT reward FROM contract_types WHERE name=?", (self.type_var.get(),))
                if row: reward = row[0][0]

            c_type = self.type_var.get()
            if c_type == "TYPE": c_type = None
            ...
            self.controller.db.cursor.execute(
                ...
                (target, self.client_in.get().upper(), reward, datetime.now().strftime("%d/%m %H:%M"), self.prio_var.get(), c_type)
            )
            self.controller.db.conn.commit()
            self.refresh()
            self.target_in.delete(0, 'end'); self.client_in.delete(0, 'end'); self.reward_in.delete(0, 'end')

    def refresh(self):
        for w in self.active_scroll.winfo_children(): w.destroy()
        for w in self.history_scroll.winfo_children(): w.destroy()
        
        total_gains = 0

        # 1. Missions OPEN
        self.controller.db.cursor.execute("SELECT * FROM contracts WHERE status='OPEN' ORDER BY id DESC")
        for row in self.controller.db.cursor.fetchall():
            ...
            ctk.CTkLabel(f, text=f"[{prio[:3]}] {row[5] if row[5] else 'TASK'}", 
                         font=DrakeConfig.FONT_LOGS, text_color=p_color).pack(side="left", padx=10)
            
            ctk.CTkLabel(f, text=f"🎯 {row[1]} | {row[3]} aUEC", 
                         font=DrakeConfig.FONT_LOGS, text_color=DrakeConfig.TEXT_MAIN).pack(side="left", padx=5)
            
            DrakeButton(f, text="CLOSE", width=60, height=24, 
                        command=lambda cid=row[0], tgt=row[1]: self.complete_contract(cid, tgt)).pack(side="right", padx=10)

        # 2. Historique CLOSED
        self.controller.db.cursor.execute("SELECT * FROM contracts WHERE status='CLOSED' ORDER BY id DESC")
        for row in self.controller.db.cursor.fetchall():
            f = ctk.CTkFrame(self.history_scroll, fg_color="transparent")
            f.pack(pady=1, fill="x", padx=5)
            
            try: total_gains += int(str(row[3]).replace(" ", "").replace(",", ""))
            except: pass
            ...
            ctk.CTkButton(f, text="[DEL]", fg_color="transparent", text_color="#444", 
                          hover_color=DrakeConfig.ACCENT_ERROR, width=20, height=18, 
                          command=lambda cid=row[0]: self.delete_history(cid)).pack(side="right", padx=5)

        self.stats_label.configure(text=f"TOTAL COLLECTED : {total_gains:,} aUEC".replace(",", " "))

    def complete_contract(self, cid, target):
        self.controller.db.cursor.execute("UPDATE contracts SET status='CLOSED' WHERE id=?", (cid,))
        # On met à jour les stats de la cible
        self.controller.db.cursor.execute("UPDATE targets SET wins = wins + 1 WHERE pseudo=?", (target,))
        if self.controller.db.cursor.rowcount == 0:
            self.controller.db.cursor.execute("INSERT INTO targets (pseudo, wins, date, alignment) VALUES (?,?,?,'ENNEMI')", 
    ...
    def apply_type(self, choice):
        ...
        row = self.controller.db.query("SELECT reward FROM contract_types WHERE name=?", (choice,))
        if row:
    ...
    def open_type_manager(self):
        ...
        ctk.CTkLabel(toplevel, text="REGISTER NEW CONTRACT TYPE", font=DrakeConfig.FONT_UI, 
                     text_color=DrakeConfig.ACCENT_PRIMARY).pack(pady=10)
        
        f_in = ctk.CTkFrame(toplevel, fg_color="transparent")
        f_in.pack(pady=5)
        
        n_entry = ctk.CTkEntry(f_in, placeholder_text="CODE (ex: VHRT)", fg_color=DrakeConfig.BG_TERMINAL)
        n_entry.pack(side="left", padx=5)
        r_entry = ctk.CTkEntry(f_in, placeholder_text="REWARD", width=100, fg_color=DrakeConfig.BG_TERMINAL)
        ...
        def add():
            ...
            if n and r:
                try:
                    self.controller.db.commit("INSERT INTO contract_types VALUES (?,?)", (n, r))
                    refresh_list()
                    self.update_type_menu()
                    n_entry.delete(0, "end"); r_entry.delete(0, "end")
                except: pass

        DrakeButton(f_in, text="+", width=40, command=add).pack(side="left", padx=5)
        
        scroll = ctk.CTkScrollableFrame(toplevel, label_text="REGISTERED TYPES", 
                                        fg_color=DrakeConfig.BG_TERMINAL, label_text_color=DrakeConfig.TEXT_SECONDARY)
        ...
        def refresh_list():
            ...
            for row in self.controller.db.query("SELECT * FROM contract_types"):
                ...
                ctk.CTkLabel(f, text=f"{row[0]} >> {row[1]} aUEC", font=DrakeConfig.FONT_LOGS).pack(side="left", padx=10)
                ctk.CTkButton(f, text="REMOVE", width=60, fg_color="transparent", text_color=DrakeConfig.ACCENT_ERROR, 
                              hover_color="#330000", command=lambda n=row[0]: delete(n)).pack(side="right", padx=5)
