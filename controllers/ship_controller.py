...
class ShipController:
    ...
    def update_ship_specs(self, ship_name, data):
        ...
        self.app.query(sql, tuple(values))
...
class ShipController:
    ...
    def mount_component(self, ship_name, category, slot_index, component_name):
        """Enregistre ou met à jour un composant dans un slot spécifique."""
        try:
            # On vérifie si c'est "EMPTY" pour supprimer l'entrée
            if component_name == "EMPTY":
                ...
                # On utilise un REPLACE (ou INSERT/UPDATE) pour synchroniser le slot
                self.db.query(
            ...
            print(f"Erreur Sync Database: {e}")
            return False
        
    def get_compatible_components(self, category, max_size):
        """Récupère les noms des composants filtrés par type et taille."""
        query = "SELECT name FROM components WHERE category = ? AND size <= ?"
        # REMPLACER self.db.query PAR self.app.query
        rows = self.app.query(query, (category, max_size)) 
        return [r[0] for r in rows]

    def get_slot_data(self, ship_name, category, max_size, slot_index):
        """Prépare les données pour la vue."""
        # 1. Récupérer les choix possibles
        available = self.get_compatible_components(category, max_size)
        
        # 2. Récupérer l'équipement actuel
        sql = """SELECT component_name FROM ship_loadout 
