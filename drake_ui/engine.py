...
- `DrakeClearButton`   : bouton reset rouge
- `DrakeEntry`         : champ texte pré-stylisé
- `DrakeEntryLight`    : variante légère de DrakeEntry
- `DrakeComboBox`      : liste déroulante custom avec dropdown
- `DrakeComboBoxLight` : variante légère de DrakeComboBox
- `DrakeDualComboBox`  : deux combobox côte à côte (double rôle)
- `DrakeSuggestionManager` : suggestions autocomplete attachées à un Entry
- `DrakeTerminal`      : zone texte / console stylisée
- `DrakePopup`         : popups modales info / error / warning / yesno
- `DrakeTitle1–4`      : labels de titre hiérarchiques
- `DrakeApp`           : application de démonstration
--- Composants Ship Loadout ---
- `DrakeSegBar`        : barre de segments ■/□ lecture seule
- `DrakeSegSelector`   : sélecteur de segments cliquables (simulateur énergie)
- `DrakeSpecBadge`     : badge de spécialisation coloré (Military/Civilian/…)
- `DrakePowerBar`      : barre d'énergie OK/OVERLOAD avec ratio dynamique
"""
...
# ==========================================
# NOUVEAUX COMPOSANTS — SHIP LOADOUT UI
# ==========================================

class DrakeSegBar(ctk.CTkLabel):
    """Affiche une barre de segments ■/□ en lecture seule.

    Usage :
        bar = DrakeSegBar(parent, value=3, total=6)
        bar.pack()
        bar.set(5)          # met à jour dynamiquement
        bar.get() → 5
    """

    def __init__(self, master, value: int = 0, total: int = 6,
                 filled_char: str = "■", empty_char: str = "□", **kwargs):
        ...
        defaults.update(kwargs)
        super().__init__(master, text=self._render(), **defaults)

    def _render(self) -> str:
        v = max(0, min(self._value, self._total))
        return self._filled * v + self._empty * (self._total - v)
...
class DrakeSegSelector(ctk.CTkFrame):
    """Sélecteur de segments cliquables (1 carré = 1 segment).

    Cliquer sur le carré N active les N premiers carrés.
    Cliquer sur le dernier actif le désactive (décrémente).

    Usage :
        sel = DrakeSegSelector(parent, total=6, value=6,
                               on_change=lambda v: print(v))
        sel.pack()
        sel.get() → int   (nombre de segments actifs)
        sel.set(3)        → met à jour visuellement
    """

    def __init__(self, master, total: int = 6, value: int = None,
                 on_change: Callable[[int], None] = None,
                 btn_size: int = 22, **kwargs):
        super().__init__(master, fg_color="transparent", **kwargs)
        self._total     = max(1, int(total))
        self._value     = int(value) if value is not None else self._total
        self._on_change = on_change
        self._btns: list = []
        self._build(btn_size)

    def _build(self, btn_size: int) -> None:
        for i in range(1, self._total + 1):
            b = ctk.CTkButton(
                self, text="■", width=btn_size, height=btn_size,
        ...
        for i, b in enumerate(self._btns, start=1):
            b.configure(command=self._make_click(i))

        self._refresh()

    def _make_click(self, idx: int) -> Callable:
        def _cb():
            self._value = idx if self._value != idx else max(0, idx - 1)
            self._refresh()
            if self._on_change:
...
class DrakeSpecBadge(ctk.CTkLabel):
    """Badge de spécialisation coloré (Military / Civilian / Industrial / Stealth).

    Usage :
        badge = DrakeSpecBadge(parent, spec="Military")
        badge.pack()
        badge.set("Stealth")
    """
    ...
    def __init__(self, master, spec: str = "", **kwargs):
        color = self._color_for(spec)
        defaults = {
            "font": DrakeConfig.FONT_LOGS,
            "text_color": color,
            "anchor": "w",
        ...
        defaults.update(kwargs)
        super().__init__(master, text=self._text_for(spec), **defaults)
        self._spec = spec

    def _color_for(self, spec: str) -> str:
        return self._COLORS.get((spec or "").strip().title(), DrakeConfig.TEXT_SECONDARY)

    def _text_for(self, spec: str) -> str:
        s = (spec or "").strip().upper()
        return (self._PREFIX + s) if s else ""

    def set(self, spec: str) -> None:
        self._spec = spec
        self.configure(text=self._text_for(spec), text_color=self._color_for(spec))

    def get(self) -> str:
        return self._spec


class DrakePowerBar(ctk.CTkFrame):
    """Barre d'énergie animée avec statut OK / OVERLOAD.

    Affiche une barre ■/□ proportionnelle plus un label de statut.

    Usage :
        bar = DrakePowerBar(parent, available=7.8)
        bar.pack(fill="x")
        bar.update(draw=5.2)   → recalcule et rafraîchit
    """

    _BAR_CELLS = 20

    def __init__(self, master, available: float = 0.0, draw: float = 0.0, **kwargs):
        super().__init__(
            master,
            fg_color=DrakeConfig.BG_PANEL,
            corner_radius=0,
    ...
    def _render(self) -> Tuple[str, str]:
        ...
        draw  = self._draw
        if avail <= 0:
            return "─  NO POWER PLANT  ─", DrakeConfig.TEXT_SECONDARY

        ratio  = min(draw / avail, 1.5)
        filled = min(self._BAR_CELLS, round(ratio * self._BAR_CELLS))
        bar    = "■" * filled + "□" * (self._BAR_CELLS - filled)
        pct    = (draw / avail) * 100

        if draw <= avail:
            color  = "#44cc44"
            status = f"OK  {draw:.2f} / {avail:.2f} seg"
        else:
            color  = "#ff4444"
            over   = draw - avail
            status = f"OVERLOAD +{over:.2f} seg  {draw:.2f} / {avail:.2f}"

        return f"[{bar}]  {pct:.0f}%  {status}", color

    def update(self, draw: float = None, available: float = None) -> None:
        """Met à jour les valeurs et rafraîchit l'affichage."""
        if draw      is not None: self._draw      = float(draw)
        if available is not None: self._available = float(available)
        text, color = self._render()
        self._label.configure(text=text, text_color=color)

    def set_available(self, value: float) -> None:
        self.update(available=value)

    def set_draw(self, value: float) -> None:
        self.update(draw=value)